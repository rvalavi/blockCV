---
title: "3. Block cross-validation for species distribution modelling"
...
output: 
  rmarkdown::html_vignette:
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{3. Block cross-validation for species distribution modelling}
...
Here, we generate two CV strategies, one k-fold CV using `cv_spatial` and one LOO CV using `cv_nndm`. See more options and configurations in [Tutorial 1 - introduction to `blockCV`](tutorial_1.html).
...
Now, let's create LOO CV folds with nearest neighbour distance matching (NNDM; Milà et al. 2022) algorithm. To run `cv_nndm`, you need a measure of spatial autocorrelation present in your data. This can be done by fitting a model, adding its residuals to the sample points object, and using that residual column to calculate spatial autocorrelation. The autocorrelation of the response variable can also be used as an exploratory proxy (Milà et al, 2022; Roberts et al. 2017), but it is not the residual autocorrelation range recommended by Roberts et al. (2017) and may produce a different distance. Here, we calculate spatial autocorrelation range in the response using the `cv_spatial_autocor` function.
