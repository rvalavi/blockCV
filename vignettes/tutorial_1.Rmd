...
This tutorial demonstrates `cv_spatial`, `cv_cluster`, `cv_buffer`, `cv_nndm`, and `cv_knndm`, plus tools for plotting folds, checking environmental similarity, and selecting spatial block sizes.
...
Optionally, all folds can be plotted in a single panel, with colours indicating different folds. This option is only available for k-fold cross-validation methods: `cv_spatial`, `cv_cluster`, and `cv_knndm.`
...
## Fold quality diagnostics

### Environmental similarity diagnostics

The `cv_similarity` function evaluates environmental similarity between training and testing folds, helping to detect potential extrapolation in the testing data. It supports MESS and L1/L2 distance-based similarity scores. Negative values indicate potential environmental novelty. Each fold is annotated with the percentage of its testing points that are novel (shown in red when a fold extrapolates), and a per-fold summary table is attached to the returned plot as `attr(p, "extrapolation")`.


```{r fig.height=4, fig.width=6}
sim <- cv_similarity(cv = ecv, # the environmental clustering
                     x = pa_data,
                     r = rasters,
                     method = "MESS",
                     progress = FALSE)
...
# the per-fold extrapolation summary (number of test points, % novel,
# min/median similarity, and the most limiting variable for MESS)
attr(sim, "extrapolation")
```

Setting `type = "map"` shows *where* the extrapolation happens: the sample points are drawn in geographical space and each testing point is coloured by its similarity value, so novel (negative) areas stand out.
...
The `cv_distance` function checks how closely a generated CV design matches the nearest-neighbour distance distribution that will be encountered when predicting across the target area. It can be run on any `blockCV` object after the folds have been created. The CV curve is compared against the prediction-to-sample distance curve, a leave-one-out reference, and, for k-fold objects, a random k-fold baseline. Lower Wasserstein-1 distances in the subtitle indicate a closer match to the prediction domain.

Set `plot = FALSE` to save the returned `ggplot` objects and compare multiple CV strategies in one figure:
...
cowplot::plot_grid(dist_sb, dist_scv, dist_bloo, dist_knn, ncol = 2)
...
Alternatively, only use the point data with `x` and `column`. This could be a binary or continuous response, or a column of residuals from a fitted model that you added to the sample points `sf` object.
