...
The `cv_similarity` function evaluates environmental similarity between training and testing folds, helping to detect potential extrapolation in the testing data. It supports three similarity measures: Multivariate Environmental Similarity Surface (MESS), Manhattan distance (L1), and Euclidean distance (L2). Negative values indicate potential environmental novelty. Each fold is annotated with the percentage of its testing points that are novel (shown in red when a fold extrapolates), and a per-fold summary table is attached to the returned plot as `attr(p, "extrapolation")`.
