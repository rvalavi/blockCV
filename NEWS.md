...
- Breaking API change: `cv_similarity()` and `cv_distance()` now return diagnostic S3 objects rather than direct `ggplot` objects. Use `$plot` or `plot()` for the plot, and use the returned diagnostic fields (`$extrapolation` and `$overall` for `cv_similarity()`; `$distances` and `$W` for `cv_distance()`) for tabular results. Code that previously customised or stored the direct ggplot return value must now access `$plot`.
