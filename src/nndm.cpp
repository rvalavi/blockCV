...
Rcpp::NumericMatrix nndm_cpp(NumericMatrix X,
  ...
  for(int i=0; i<X_cpp.nrow() ;i++){
    ...
    for(int j=0; j<X_cpp.ncol() ;j++){
      if((X_cpp(i, j) != na_value) && X_cpp(i, j) < minimum_i) {
        minimum_i = X_cpp(i, j);
        Gjstar_indices[i] = j;
      }
    }
  ...
  while( rmin <= phi ) {
    ...
    if( (tcdf >= pcdf) && in_prop ) {
      ...
      for(int i=0; i<X_cpp.nrow() ;i++){
        ...
        for(int j=0; j<X_cpp.ncol() ;j++){
          if((X_cpp(i, j) != na_value) && X_cpp(i, j) < running_minimum) {
            running_minimum = X_cpp(i, j);
            Gjstar_indices[i] = j;
          }
        }
        if(running_minimum >= rmin && running_minimum < new_rmin) {
          new_rmin = running_minimum;
          imin = i;
        }
      }
